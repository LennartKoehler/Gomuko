...
void GameState::placePieceRequestInplace(int i, int j, int playerID){
    if (getValue(i, j) == 0){ // check if viable move
        placePiece(i, j, playerID);
    }
}

Matrix<int> GameState::placePieceRequest(int i, int j, int playerID){
    if (getValue(i, j) == 0){ // check if viable move
        Matrix<int> copy{stateMatrix};
        copy.setValue(i, j, playerID);
        return copy;
    }
    return stateMatrix;
}
