...
public:
    ...
    Matrix<T> rotate90() const {
        ...
        for (const auto& me : *this){
            result.setValue(me.getCol(), this->get_num_rows() - 1 - me.getRow(), me.getValue());
        }
        return result;
    }

    Matrix<T> mirrorHorizontal() const {
        Matrix<T> result{*this};
        for (const auto& me : *this){
            result.setValue(this->get_num_rows() - me.getRow() - 1, me.getCol(), me.getValue());
        }
        return result;
    }

    Matrix<T> mirrorVertical() const {
        Matrix<T> result{*this};
        for (const auto& me : *this){
            result.setValue(me.getRow(), this->get_num_cols() - me.getCol() - 1, me.getValue());
        }
        return result;
    }
