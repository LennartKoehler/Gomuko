...
private:
	...
	void PollIncomingMessages()
	{
		...
		while ( !g_bQuit )
		{
			...
			Action* p_incoming = (Action*)pIncomingMsg->m_pData;
			Printf("recieved action", *p_incoming);
...
private:
	...
	void PollLocalUserInput()
	{
		...
		while ( !g_bQuit && LocalUserInput_GetNext( cmd ))
		{
			...
			Action test = Action(1,2,3);
			m_pInterface->SendMessageToConnection( m_hConnection, &test, (uint32)3*4, k_nSteamNetworkingSend_Reliable, nullptr ); //IMPORTANT SEND MESSAGE
